<template>
  <div class="documents__view">
    <HeaderComponent />
    <div id="documents" class="w-full bg-violet-100 py-10 document__container">
      <!-- LOADING DOCUMENTS  -->
      <div v-if="loading" class="loading">
        <button
          type="button"
          class="inline-flex items-center px-4 py-2 font-semibold leading-6 text-sm shadow rounded-md text-white bg-primary-custom transition ease-in-out duration-150 cursor-not-allowed"
          disabled=""
        >
          <svg
      ...
      <div v-else class="max-w-7xl mx-auto container">
        <div class="w-full flex flex-col md:justify-between md:flex-row omnisearch">
          ...
          <div class="w-full md:w-auto md:flex search">
            <input
              ...
              @keyup.enter="search"
              type="text"
              class="px-2 rounded-md border md:max-w-sm lg:max-w-xl"
            />
            <label
              @click="search()"
              for="search"
              class="hidden md:flex h-10 bg-primary-custom ml-4 px-4 rounded-sm justify-center text-white items-center cursor-pointer"
              >Buscar</label
            >
          </div>
        </div>
        ...
        <div
          ...
          id="documentsList"
          class="overflow-y-auto overflow-x-auto w-7xl h-auto max-h-[60rem]"
        >
          <table class="w-full snap-y relative">
            <tr
              class="bg-primary-custom text-white h-12 items-center flex w-full top-0 left-0 right-0 sticky"
            >
            ...
            <tr
              ...
              :key="document.id"
              :class="[index % 2 == 0 ? 'bg-gray-200 text-gray-900' : 'bg-white']"
              class="snap-start flex h-12 items-center w-full document"
            >
              ...
              <td class="w-custom">{{ document.attributes.creationDate | moment }}</td>
              <td class="w-custom">{{ document.attributes.assignmentDate | moment }}</td>
              <td
                @click="toggleDocument(document.id, document.attributes.ownerFullName)"
                class="w-custom flex justify-end pr-10 cursor-pointer"
              >
                <svg
                  width="20"
                  height="20"
        ...
        <div v-else>No existen documentos filtrados por "{{ searchConst }}"</div>
      </div>
